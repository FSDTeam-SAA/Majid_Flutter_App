...
class SalesTrendChart extends StatelessWidget {
  ...
  const SalesTrendChart({super.key, required this.thisMonth, required this.lastMonth});
        ...
        children: [
          Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(border: Border.all(color: AppColors.fieldBorder), borderRadius: BorderRadius.circular(20)),
          ...
          SizedBox(
            height: 160,
            child: CustomPaint(size: const Size(double.infinity, 160), painter: _ChartPainter(thisMonth: thisMonth, lastMonth: lastMonth)),
          ),
...
class _ChartPainter extends CustomPainter {
  ...
  void _drawLine(Canvas canvas, Size size, List<double> data, Color color, double minVal, double maxVal, {bool drawDots = false}) {
    final paint = Paint()..color = color..strokeWidth = 2.5..style = PaintingStyle.stroke..strokeCap = StrokeCap.round..strokeJoin = StrokeJoin.round;
    final n = data.length;
    final path = Path();
    ...
    if (drawDots) {
      for (int i = 0; i < n; i++) {
        ...
        canvas.drawCircle(Offset(x(i), y(i)), 4, Paint()..color = const Color(0xFF131F1C)..style = PaintingStyle.stroke..strokeWidth = 2);
      }
    }
  }
